...
const access_logger = require('simple-node-logger').createSimpleFileLogger(config.access.accountFile);
...
function sendRequest(req, res, next) {
    ...
    req.fwd_request = request(options).on('error', function handleConnectionError(e) {
...
function accountInfo(req, res, next) {
    req.fwd_request = req.fwd_request.on("response", function(res) {
        access_logger.info("StatusCode: ", res.statusCode,
                           " UserId: ", req.userId,
