...
/**
 * Middleware to extract the organization data from the headers.
 *
...
/**
 * Middleware to extract the user data (usually a token) from the headers.
 *
 * @param {Object} req           Incoming headers.
 * @param {Object} res           Outgoing response.
 * @param {Function} next        Call to the next middleware in the chain.
...
/**
 * Generates the FRN to identify the target resource of the headers. The FRN is composed of information from the
...
/**
 * Redirects the incoming headers to the proxied host. The headers is not read with a pipe, as it has been completely
...
function sendRequest(req, res, next) {
    ...
    request(options).on('error', function handleConnectionError(e) {
        logger.error('Error forwarding the headers to target proxy: %s', e.message);
