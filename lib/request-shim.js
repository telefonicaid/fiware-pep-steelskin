...
function request(options, callback) {
    ...
    const safeCallback = function(...args) {
        if (callbackCalled) {
            console.log('WARNING: Callback already called, ignoring duplicate call');
            return;
        }
        callbackCalled = true;
    ...
    console.log('MAKING REQUEST: URL=%s Method=%s', options.url || options.uri, options.method || 'GET');
    console.log('HTTP OPTIONS: %s', JSON.stringify(httpOptions, null, 2));
    
    got(options.url || options.uri, httpOptions)
        .then((response) => {
            console.log('GOT SUCCESS: URL=%s Status=%s', options.url || options.uri, response.statusCode);
            try {
                console.log('Response headers exist: %s', !!response.headers);
                console.log('Response body type: %s', typeof response.body);
                ...
                if (typeof responseBody === 'string' && response.headers['content-type']) {
                    ...
                    if (contentType.includes('application/json')) {
                        try {
                            ...
                            console.log('SUCCESS: Auto-parsed JSON response body');
                        } catch (e) {
                            console.log('SUCCESS: Failed to auto-parse JSON response: %s, body: %s', e.message, responseBody);
                            // Keep original body if parsing fails
                        }
                    }
                } else if (typeof responseBody === 'object' && responseBody !== null) {
                    // If responseBody is already an object (from got's JSON parsing), keep it as is
                    console.log('Response body is already an object, keeping as is');
                }
                
                console.log('About to create requestStyleResponse');
                ...
                console.log('About to call safeCallback');
                logger.debug('SUCCESS calling callback with: error=null, statusCode=%s', requestStyleResponse.statusCode);
                
                // Match original request library interface: callback(error, response, body)
                safeCallback(null, requestStyleResponse, responseBody);
                console.log('safeCallback completed successfully');
            } catch (error) {
                console.log('Error in then handler: %s', error.message);
                // Don't re-throw, just call safeCallback which will ignore if already called
                safeCallback(error, null, null);
            }
        })
        .catch((error) => {
            console.log('GOT ERROR: URL=%s', options.url || options.uri);
            ...
            console.log('ERROR details: %s', JSON.stringify(errorInfo, null, 4));
            ...
            if (error.response) {
                ...
                return safeCallback(null, requestStyleResponse, error.response.body);
            } else {
                ...
                logger.debug('ERROR without response: %s', JSON.stringify({
                    error: error.message, 
                    code: error.code, 
                    name: error.name, 
                ...
                return safeCallback(error, minimalResponse, undefined);
            }
        });
}
