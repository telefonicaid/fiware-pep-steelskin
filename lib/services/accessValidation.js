...
function getRolesFromResponse(rawBody, organization) {
    ...
    if (roles.length === 0) {
        for (var j=0; j < body.roles.length; j++) {
            roles.push(body.organizations[i].roles[j].id);
        }
    }
...
function authenticateProxy(req, res, next) {
    ...
    request(options, function(error, response, body) {
        if (error) {
            logger.error('[VALIDATION-GEN-003] Error connecting the Keystone Proxy for authentication: %s', error.message);
            next(new errors.KeystoneProxyAuthenticationError(error));
        } else if (response.statusCode === 200) {
            ...
            logger.error('[VALIDATION-GEN-004] Authentication to the Keystone Proxy rejected with code %s', response.statusCode);
            logger.debug('Error payload: \n%s\n\n', JSON.stringify(body, null, 4));
            next(new errors.KeystoneProxyAuthenticationRejected(response.statusCode));
        }
    });
