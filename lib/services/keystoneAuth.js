...
function getRolesFromResponse(rawBody, organization, cacheKey) {
    ...
    for (var i = 0; i < body.role_assignments.length; i++) {
        if (body.role_assignments[i].scope &&
            ...
            String(body.role_assignments[i].scope.project.id) === String(organization)) {
...
function retrieveRoles(req, callback) {
    ...
    cacheAndHold('roles', cacheKey, retrieveRequest, processValue, callback);
}
...
function retrieveUser(req, callback) {
    ...
    cacheAndHold('user', userToken, retrieveRequest, processValue, callback);
}
