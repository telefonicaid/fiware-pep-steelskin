...
function getRolesFromResponse(rawBody, organization, cacheKey) {
    ...
    function retrieveRequest(innerCb) {
        ...
        request(options, function retrieveRolesHandler(error, response, body) {
            ...
            if (error) {
                ...
                innerCb(new errors.KeystoneProxyAuthenticationError(message));
            } else if (response.statusCode === 200) {
                logger.debug('Roles response from Keystone: \n%s\n\n', JSON.stringify(body, null, 4));
                innerCb(null, getRolesFromResponse(response.body, req.subserviceId, cacheKey));
            } else {
                currentToken = null;
                logger.error('Role extraction from Keystone rejected with code %s', response.statusCode);
                logger.debug('Error payload: \n%s\n\n', JSON.stringify(body, null, 4));
                innerCb(new errors.KeystoneProxyAuthenticationError(500));
            }
        });
    }

    var cachedValue = rolesCache.get(cacheKey);

    if (cachedValue[cacheKey]) {
        processValue(cachedValue[cacheKey].roles, callback);
    } else if (cache.updating.roles) {
        cache.channel.on('roles', callback);
    } else {
        cache.updating.roles = true;
        cache.channel.removeAllListeners('roles');
        cache.channel.on('roles', function(error, value) {
        ...
        async.waterfall([
            ...
            cache.channel.emit('roles', error, value);
        });
    }
}
...
function retrieveUser(req, callback) {
    ...
    function retrieveRequest(innerCb) {
        ...
        request(options, function(error, response, body) {
            ...
            if (error) {
                ...
                innerCb(new errors.KeystoneProxyAuthenticationRejected(404));
                ...
                innerCb(new errors.KeystoneProxyAuthenticationRejected(401));
            } else if (response.statusCode === 200) {
                var cachedValue = {
    ...
    if (cachedValue[userToken]) {
        processValue(cachedValue[userToken], callback);
    } else if (cache.updating.user) {
        cache.channel.on('user', callback);
    } else {
        cache.updating.user = true;
        cache.channel.removeAllListeners('user');
        cache.channel.on('user', function(error, value) {
            callback(error, value);
        });
        ...
        async.waterfall([
            ...
            cache.channel.emit('user', error, value);
        });
    }
}
...
function retrieveSubserviceId(req, callback) {
        ...
        request(options, function(error, response, body) {
            ...
            if (error) {
                ...
                if (project) {
                    ...
                    projectCache.set(cacheKey, cachedValue);
    ...
    var cachedValue = projectCache.get(cacheKey);

    if (cachedValue[cacheKey]) {
        processValue(cachedValue[cacheKey], callback);
    } else if (cache.updating.subservice) {
        cache.channel.on('subservice', callback);
    } else {
        cache.updating.subservice = true;
        cache.channel.removeAllListeners('subservice');
        cache.channel.on('subservice', function(error, value) {
            callback(error, value);
        });
        ...
        async.waterfall([
            ...
            cache.channel.emit('subservice', error, value);
        });
    }
}
