...
function retrieveRoles(req, callback) {
    ...
    var cachedValue = cache.data.roles.get(cacheKey);
...
function retrieveUser(req, callback) {
    ...
    var cachedValue = cache.data.user.get(userToken);
...
function cacheAndHold(cacheType, cacheKey, retrieveRequestFn, processValueFn, innerCb) {
    var cachedValue = cache.data[cacheType].get(cacheKey);

    if (cachedValue[cacheKey]) {
        processValueFn(cachedValue[cacheKey], innerCb);
    } else if (cache.updating[cacheType]) {
        cache.channel.on(cacheType, innerCb);
    } else {
...
function retrieveSubserviceId(req, callback) {
    ...
    cacheAndHold('subservice', cacheKey, retrieveRequest, processValue, callback);
}
