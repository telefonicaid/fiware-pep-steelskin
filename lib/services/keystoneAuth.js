...
function retrieveUser(req, callback) {
    ...
    request(options, function(error, response, body) {
        ...
        if (error) {
            ...
            req.serviceId = body.token.user.domain.id;
            req.userId = body.token.user.id;

            callback(null, body.token.user.id);
        } else {
            currentToken = null;
    ...
    if (currentToken === null) {
        ...
        request(options, function(error, response, body) {
            ...
            if (error) {
                ...
                logger.debug('Error payload: \n%s\n\n', JSON.stringify(body, null, 4));
                waitingRequests.emit('token', new errors.KeystoneProxyAuthenticationRejected(response.statusCode));
            }
        });
