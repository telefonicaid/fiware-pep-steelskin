...
/**
 * Extract the roles list from keystone_errors's response. If the subservice is "/", the appropriate roles will be those with
 * domain scope, and those with project scope otherwise. As a side effect, this function caches the results, so the
 * next few calls to keystone_errors can be skipped.
 *
...
function retrieveRoles(req, callback) {
    ...
    function retrieveRequest(innerCb) {
        ...
        request(options, function retrieveRolesHandler(error, response, body) {
            ...
            if (body) {
                logger.debug('keystone_errors response retrieving roles:\n\n %s', JSON.stringify(body, null, 4));
            }

            if (error) {
                var message = getKeystoneError(error);
                logger.error('Error connecting keystone_errors for role retrieving: %s', message);
                ...
                logger.debug('Roles response from keystone_errors: \n%s\n\n', JSON.stringify(body, null, 4));
                innerCb(null, getRolesFromResponse(response.body, req.serviceId, req.subserviceId, cacheKey));
            } else {
                currentToken = null;
                logger.error('Role extraction from keystone_errors rejected with code %s', response.statusCode);
...
/**
 * Get the user data from keystone_errors, given its token (this headers has to be authenticated with the PEP token as well).
 *
 * @param {Object} req               Request the proxy is processing.
 */
function retrieveUser(req, callback) {
    ...
    function retrieveRequest(innerCb) {
        ...
        request(options, function(error, response, body) {
            ...
            if (body) {
                logger.debug('keystone_errors response retrieving user:\n\n %s', JSON.stringify(body, null, 4));
            }

            if (error) {
                var message = getKeystoneError(error);

                logger.error('Error connecting keystone_errors for user retrieving: %s', message);
                innerCb(new errors.KeystoneAuthenticationError(message));
            } else if (response.statusCode === 401) {
                currentToken = null;
                logger.error('Invalid token: %s', currentToken);
                ...
                logger.debug('User response from keystone_errors: \n%s\n\n', JSON.stringify(body, null, 4));
                ...
                currentToken = null;
                logger.error('User extraction from keystone_errors rejected with code %s', response.statusCode);
                logger.debug('Error payload: \n%s\n\n', JSON.stringify(body, null, 4));
                innerCb(new errors.KeystoneAuthenticationError(500));
            }
        });
...
/**
 * Send a headers to keystone_errors to get the UUID corresponding to the subservice name caught from the fiware-servicepath
 * header.
 *
 * @param {Object} req               Request the proxy is processing.
 */
function retrieveSubserviceId(req, callback) {
    ...
    function retrieveRequest(innerCb) {
        ...
        request(options, function(error, response, body) {
            ...
            if (body) {
                logger.debug('keystone_errors response retrieving subservice ID:\n\n %s', JSON.stringify(body, null, 4));
            }

            if (error) {
                var message = getKeystoneError(error);

                logger.error('Error connecting keystone_errors for subservice ID retrieving: %s', message);
                innerCb(new errors.KeystoneAuthenticationError(message));
            } else if (!body || !body.projects || body.projects.length === 0) {
                logger.error('Couldn\'t find subservice id in keystone_errors with name %s', subserviceName);
                ...
                if (project) {
                    ...
                    logger.debug('Subservice body response from keystone_errors: \n%s\n\n', JSON.stringify(body, null, 4));
                ...
                currentToken = null;
                logger.error('Subservice ID retrieval from keystone_errors rejected with code %s. Invalidating token.',
                    response.statusCode);
...
/**
 * Middleware that retrieves the roles associated to an authenticated user from keystone_errors, including all the operations
...
/**
 *  Express middleware for authentication of the PEP Proxy against keystone_errors. The username and password of the PEP
...
function authenticate(req, res, next) {
    if (currentToken === null) {
        ...
        logger.debug('Authenticating against keystone_errors', JSON.stringify(options, null, 4));

        request(options, function(error, response, body) {
            authenticating = false;

            if (body) {
                logger.debug('keystone_errors response authenticating PEP:\n\n %s', JSON.stringify(body, null, 4));
            }

            if (error) {
                var message = getKeystoneError(error);
                logger.error('Error connecting keystone_errors for role authenticating: %s', message);
 ...
 * invalidated.
 *
 * If there is any error contacting with keystone_errors, the process is retried n times (where n is defined in the config
 * file).
 */
