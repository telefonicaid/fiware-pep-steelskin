...
const ACTION_MAP = {
    ORION: {
        ...
        EDITOR: ['read', 'create', 'update', 'notify', 'register',
    ...
    PERSEO: {
        CUSTOMER: ['readRule'],
        EDITOR: ['readRule', 'writeRule', 'notify'],
    ...
    try {
        ...
        for (const role of roles) {
            ...
            let match = roleInfo.match(
                /^(ServiceCustomer|ServiceAdmin|ServiceEditor|SubServiceCustomer|SubServiceAdmin|SubServiceEditor)([A-Z]+)$/i
            );
            ...
            // Case 2: role does NOT include component -> apply over all components
                ...
                match = roleInfo.match(
                    /^(ServiceCustomer|ServiceAdmin|ServiceEditor|SubServiceCustomer|SubServiceAdmin|SubServiceEditor)$/i
                );
            ...
            const ROLE_CLASS =
                  roleType.endsWith('Customer') ? 'CUSTOMER' :
                  roleType.endsWith('Editor') ? 'EDITOR' :
                  'ADMIN';
