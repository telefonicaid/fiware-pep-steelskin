...
function inspectUrl(req, res, callback) {
    ...
    if (req.url.toLowerCase().match(/\/(v1|ngsi10)\/querycontext$/)) {
        req.action = 'read';
    } else if (req.url.toLowerCase().match(/\/(v1|ngsi9)\/subscribecontextavailability$/)) {
        req.action = 'subscribe-availability';
    } else if (req.url.toLowerCase().match(/\/(v1|ngsi10)\/subscribecontext$/)) {
        req.action = 'subscribe';
    } else if (req.url.toLowerCase().match(/\/(v1|ngsi9)\/registercontext$/)) {
        req.action = 'register';
    } else if (req.url.toLowerCase().match(/\/(v1|ngsi9)\/discovercontextavailability$/)) {
        req.action = 'discover';
    } else {
