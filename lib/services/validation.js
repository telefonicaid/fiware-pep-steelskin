 ...
 * @param {String} action                Name of the action the headers is trying to execute.
 g */
function createAccessRequest(roles, organization, action, callback) {
    ...
    logger.debug('Creating access headers for user [%s], with organization [%s] and action [%s]',
...
/**
 * Sends the validation headers to the Access Control with the XML payload received.
 *
...
function sendAccessRequest(headers, accessPayload, callback) {
    ...
    logger.debug('Sending access headers: \n%s\n', JSON.stringify(options, null, 4));
...
/**
 * Launches the validation process for the incoming headers. As all the other middlewares in the proxy, it should chain
 ...
 *  proxy has started, and the templates themselves are reused for every headers.
 */
