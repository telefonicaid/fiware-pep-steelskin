...
function checkBypass(req, res, next) {
    if (config.bypass && config.adminRoleId && req.roles) {
        for (var i = 0; i < req.roles.length; i++) {
            if (req.roles[i] === config.adminRoleId) {
                req.bypass = true;
            }
        }
    }
...
function startProxy(callback) {
    ...
    proxyObj.proxy.use(authorization.authenticate);
    proxyObj.proxy.use(authorization.extractRoles);
